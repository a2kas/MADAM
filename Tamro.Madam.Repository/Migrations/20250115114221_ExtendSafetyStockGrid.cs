...
{
    /// <inheritdoc />
    public partial class ExtendSafetyStockGrid : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"ALTER VIEW [dbo].[v_SafetyStockGrid] AS
			SELECT
            ssc.Id,
            ssi.Country,
            ssc.CanBuy,
            ...
            ss.RetailQuantity,
            CASE WHEN (ssc.SafetyStockPharmacyChainGroup = 'Benu' OR sspc.DisplayName = 'Benu') AND ISNULL(ss.WholesaleQuantity,0) >= 0 THEN ISNULL(ss.WholesaleQuantity,0) ELSE 
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            ss.WholesaleQuantity,
            ss.RetailQuantity,
            CASE WHEN (ssc.SafetyStockPharmacyChainGroup = 'Benu' OR sspc.DisplayName = 'Benu') THEN ISNULL(ss.WholesaleQuantity,0) ELSE 
                CASE WHEN (ISNULL(ss.WholesaleQuantity,0)-ISNULL(ss.RetailQuantity,0)) < 0 THEN 0
                ELSE CAST(ISNULL(ss.WholesaleQuantity,0)-ROUND(ISNULL(ss.RetailQuantity,0),+1) AS INT) END 
                END as QuantityToBuy,
            ssi.ItemGroup, 
