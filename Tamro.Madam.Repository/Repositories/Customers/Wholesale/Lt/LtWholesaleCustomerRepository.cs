...
namespace Tamro.Madam.Repository.Repositories.Customers.Wholesale.Lt;

public class LtWholesaleCustomerRepository : IWholesaleCustomerRepository
{
    private readonly IWhRawLtDatabaseContext _context;
    private readonly ISksDbContext _sksDbContext;
    private readonly IMapper _mapper;

    public LtWholesaleCustomerRepository(IWhRawLtDatabaseContext context, ISksDbContext sksDbContext, IMapper mapper)
    {
        _context = context;
        _sksDbContext = sksDbContext;
        _mapper = mapper;
    }
    ...
    private IQueryable<LtWholesaleCustomer> BuildQuery(IEnumerable<int> addressNumbers, WholesaleCustomerType customerType)
    {
        var query = _context
            .Customers
            .AsNoTracking();
    ...
    private IQueryable<LtWholesaleCustomer> BuildQuery(string searchTerm, WholesaleCustomerType customerType)
    {
        var query = _context
            .Customers
            .AsNoTracking();
    ...
    private IQueryable<OrderNotificationEmail> BuildQuery(WholesaleCustomerSearchModel search)
    {
        var query = _sksDbContext
            .OrderNotificationEmails
            .AsNoTracking();

        if (search.AddressNumbers != null)
        {
            var addressNumberStrings = search.AddressNumbers.Select(num => num.ToString()).ToList();
            query = query.Where(x => addressNumberStrings.Contains(x.AddressNumber));
        }
